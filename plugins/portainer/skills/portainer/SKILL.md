...
```bash
# List containers
curl -s http://scc_contabo:9000/api/endpoints/3/docker/containers/json \
  -H "X-API-Key: {API_KEY}"

# List stacks
curl -s http://scc_contabo:9000/api/stacks \
  -H "X-API-Key: {API_KEY}"

# Restart a container
curl -s -X POST http://scc_contabo:9000/api/endpoints/3/docker/containers/{ID}/restart \
  -H "X-API-Key: {API_KEY}"

# Check endpoint health
curl -s http://scc_contabo:9000/api/endpoints \
  -H "X-API-Key: {API_KEY}"
```

## Notes
- Endpoint ID for scc_contabo local Docker is 3
